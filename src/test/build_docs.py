...
DOCS_INPUT_DIR = BASE_DIR / "./res/doc-automation/input"
DOCS_OUTPUT_DIR = BASE_DIR / "./build/doc-automation/output"

ARCHIVE_DIR = BASE_DIR / "./build/doc-automation/archive"
CACHE_FILE = BASE_DIR / "./build/doc-automation/.build_cache.json"
...
# -------------------------------------------------------
# 🔥 CENTRAL SAFE ESCAPING (FIXED + GLOBAL)
# -------------------------------------------------------

def escape_xml(text: str) -> str:
    """
    Escapes only what XML/SVG/Confluence requires.
    Safe + idempotent.
    """
        ...
        for field in REFERENCE_FIELDS:
            if field in term:
                for ref in term[field]:
                    if ref not in terms:
                        errors.append(f"{term_name}.{field} -> unknown reference '{ref}'")
...
def process_loops(text, terms, errors, local_ctx=None):
    def repl(match):
        ...
        for item in iterable:
            ...
            ctx[var_name] = item
            result.append(render(body, terms, errors, ctx).strip())
...
def process_file(input_path, output_path, terms, cache, terms_hash, force=False, dry_run=False):
    ...
    if isinstance(cache_entry, str):
        cache_entry = {"input_hash": cache_entry, "terms_hash": None}

    if (
        not force and
        cache_entry.get("input_hash") == input_hash and
        cache_entry.get("terms_hash") == terms_hash
    ):
        return errors
    ...
    # 🔥 FINAL GLOBAL SAFETY STEP (IMPORTANT FIX)
